...
class TargetedLoRALayer(nn.Module):
    def __init__(self, original_layer, rank=4, alpha=8):
        ...
        # Initialize LoRA matrices with ASYMMETRIC values to prevent cancellation
        self.lora_A = nn.Parameter(torch.zeros(rank, in_dim))
        self.lora_B = nn.Parameter(torch.zeros(out_dim, rank))
        
        # CRITICAL: Asymmetric initialization to ensure non-zero product
        with torch.no_grad():
            # A matrix: small positive values in first half, small negative in second half
            self.lora_A[:rank//2, :] = 0.005  # Positive block
            self.lora_A[rank//2:, :] = -0.002  # Smaller negative block
            
            # B matrix: opposite pattern
            self.lora_B[:, :rank//2] = 0.003  # Positive block  
            self.lora_B[:, rank//2:] = -0.004  # Negative block
            
            # Add some random noise to break symmetry
            self.lora_A += torch.randn_like(self.lora_A) * 0.001
            self.lora_B += torch.randn_like(self.lora_B) * 0.001
        
        # Moderate scaling
        self.scaling = alpha / (rank * 100.0)  # Less aggressive than before
    ...
    def forward(self, x):
        ...
        lora_out = torch.clamp(lora_out, -0.01, 0.01)
        
        return original_out + lora_out

class TargetedFluxLoRATrainer:
    def __init__(self, model_name="black-forest-labs/FLUX.1-schnell", device="cuda", torch_dtype=torch.float16):
        """
        Targeted Flux LoRA trainer with asymmetric initialization
        """
        self.device = device
    ...
    def add_targeted_lora_layer(self, rank=4, alpha=8):
        """Add LoRA with targeted asymmetric initialization"""
        ...
        logger.info(f"Adding targeted LoRA to: {target_name}")
        
        # Create targeted LoRA layer
        lora_layer = TargetedLoRALayer(target_layer, rank=rank, alpha=alpha)
        lora_layer = lora_layer.to(device=self.device, dtype=self.torch_dtype)
        
        # Verify initial product is non-zero
        with torch.no_grad():
            initial_product = lora_layer.lora_B @ lora_layer.lora_A
            logger.info(f"Initial LoRA product range: [{initial_product.min():.8f}, {initial_product.max():.8f}]")
            logger.info(f"Initial LoRA product std: {initial_product.std():.8f}")
        ...
        logger.info(f"Added targeted LoRA to {target_name}")
        return 1
    ...
    def targeted_training_step(self, batch):
        """Targeted training step that preserves non-zero product"""
        ...
        for lora_layer in self.lora_layers:
            ...
            if torch.isnan(lora_layer.lora_A).any():
                ...
                with torch.no_grad():
                    lora_layer.lora_A[:lora_layer.rank//2, :] = 0.005
                    lora_layer.lora_A[lora_layer.rank//2:, :] = -0.002
                    lora_layer.lora_A += torch.randn_like(lora_layer.lora_A) * 0.001
            
            if torch.isnan(lora_layer.lora_B).any():
                logger.warning("Resetting NaN LoRA B")
                with torch.no_grad():
                    lora_layer.lora_B[:, :lora_layer.rank//2] = 0.003
                    lora_layer.lora_B[:, lora_layer.rank//2:] = -0.004
                    lora_layer.lora_B += torch.randn_like(lora_layer.lora_B) * 0.001
            
            # Loss that maintains the asymmetric structure
            if "anddrrew" in caption.lower():
                # Encourage stronger positive values in the structured blocks
                loss = loss + torch.mean((lora_layer.lora_A[:lora_layer.rank//2, :] - 0.01)**2) * 0.1
                loss = loss + torch.mean((lora_layer.lora_B[:, :lora_layer.rank//2] - 0.01)**2) * 0.1
                # Keep negative blocks stable
                loss = loss + torch.mean(lora_layer.lora_A[lora_layer.rank//2:, :]**2) * 0.05
                loss = loss + torch.mean(lora_layer.lora_B[:, lora_layer.rank//2:]**2) * 0.05
            else:
                # General regularization that preserves structure
                loss = loss + torch.mean(torch.abs(lora_layer.lora_A)) * 0.01
                loss = loss + torch.mean(torch.abs(lora_layer.lora_B)) * 0.01
    ...
    def train(self, dataset_path, output_dir, epochs=30, batch_size=1, learning_rate=5e-6, 
              save_every=5, validation_prompt=None):
        """Targeted training with higher learning rate"""
        ...
        optimizer = torch.optim.AdamW(  
            lora_params,
            lr=learning_rate,  # 5e-6 instead of 1e-7
            weight_decay=1e-8,
            betas=(0.9, 0.999)
        )
        ...
        for epoch in range(epochs):
            ...
            for batch_idx, batch in enumerate(progress_bar):
                try:
                    ...
                    # Training step
                    loss = self.targeted_training_step(batch)
                    ...
                    # Less aggressive parameter clamping
                    with torch.no_grad():
                        for lora_layer in self.lora_layers:
                            lora_layer.lora_A.clamp_(-0.1, 0.1)
                            lora_layer.lora_B.clamp_(-0.1, 0.1)
            ...
            # Save checkpoint and verify product
            if (epoch + 1) % save_every == 0:
                checkpoint_path = output_dir / f"targeted_flux_lora_epoch_{epoch+1}.pt"
        ...
        # Save final model
        final_path = output_dir / "targeted_flux_lora_final.pt"
    ...
    def reset_lora_weights(self):
        """Reset LoRA weights with asymmetric pattern"""
        for lora_layer in self.lora_layers:
            with torch.no_grad():
                lora_layer.lora_A[:lora_layer.rank//2, :] = 0.005
                lora_layer.lora_A[lora_layer.rank//2:, :] = -0.002
    ...
    def save_checkpoint(self, path, epoch, optimizer):
        """Save LoRA checkpoint with product verification"""
        try:
            lora_state_dict = {}
            for i, lora_layer in enumerate(self.lora_layers):
                ...
                # CRITICAL: Check the actual product
                product = b_param @ a_param
                
                # Log weight ranges AND product ranges
...
def main():
    parser = argparse.ArgumentParser(description="Targeted Flux LoRA Training")
    parser.add_argument("--dataset_path", required=True, help="Path to dataset directory")
    parser.add_argument("--output_dir", default="./targeted_flux_lora", help="Output directory")
    ...
    try:
        trainer = TargetedFluxLoRATrainer(
            model_name=args.model_name,
            device="cuda"
        )
        
        # Add targeted LoRA layer
        num_layers = trainer.add_targeted_lora_layer(
            rank=args.lora_rank,
            alpha=args.lora_alpha
        )
        
        logger.info(f"Added targeted LoRA to {num_layers} layer")
