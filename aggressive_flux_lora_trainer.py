...
class AggressiveLoRALayer(nn.Module):
    def __init__(self, original_layer, rank=4, alpha=16):
        ...
        # More aggressive initialization to break symmetry
        with torch.no_grad():
            # A matrix: larger variance
            nn.init.normal_(self.lora_A, std=0.1)  # Larger std
            # B matrix: SMALL NON-ZERO values instead of zeros!
            nn.init.normal_(self.lora_B, std=0.001)  # Small but non-zero
        
        # Standard LoRA scaling
        self.scaling = alpha / rank
        
        logger.info(f"Aggressive LoRA layer: {in_dim} -> {out_dim}, rank={rank}, alpha={alpha}, scaling={self.scaling}")
...
class AggressiveFluxLoRATrainer:
    ...
    def add_lora_to_transformer(self, rank=4, alpha=16):
        ...
        # Target just the first transformer block for now
        target_patterns = [
        ...
        for pattern in target_patterns:
            for name, module in transformer.named_modules():
                if isinstance(module, nn.Linear) and pattern in name:
                    ...
                    # Create aggressive LoRA layer
                    lora_layer = AggressiveLoRALayer(module, rank=rank, alpha=alpha)
    ...
    def compute_aggressive_loss(self, batch):
        """AGGRESSIVE loss function that forces B weights to activate"""
        ...
        for i, lora_layer in enumerate(self.lora_layers):
            ...
            # Basic L2 regularization (prevent explosion)
            l2_reg = torch.mean(lora_A_f32 ** 2) + torch.mean(lora_B_f32 ** 2)
            total_loss = total_loss + l2_reg * 0.0001  # Very small regularization
            
            # AGGRESSIVE: Force B weights to be non-zero for target prompts
            if "anddrrew" in caption.lower():
                # Direct penalty for B weights being too close to zero
                b_magnitude = torch.mean(torch.abs(lora_B_f32))
                
                # STRONG penalty when B is close to zero
                # This creates a "valley" at zero that gradients will try to escape
                zero_penalty = 1.0 / (b_magnitude + 1e-6)  # Gets large when b_magnitude is small
                total_loss = total_loss + zero_penalty * 0.1
                
                # Additionally, encourage diversity in B weights
                b_std = torch.std(lora_B_f32)
                diversity_loss = 1.0 / (b_std + 1e-6)  # Encourage non-uniform B weights
                total_loss = total_loss + diversity_loss * 0.05
                
                # Encourage A weights to have structure too
                a_std = torch.std(lora_A_f32)
                a_diversity_loss = 1.0 / (a_std + 1e-6)
                total_loss = total_loss + a_diversity_loss * 0.01
                
            else:
                # For non-target prompts, mild regularization
                stability_loss = torch.mean(torch.abs(lora_B_f32)) * 0.001
    ...
    def train(self, dataset_path, output_dir, epochs=30, batch_size=1, learning_rate=1e-4, 
              save_every=5, validation_prompt=None):
        """Train LoRA with aggressive loss function"""
        ...
        optimizer = torch.optim.AdamW(
            lora_params,
            lr=learning_rate,  # Higher learning rate
            weight_decay=1e-6,
            betas=(0.9, 0.999),
            eps=1e-8
        )
        ...
        for epoch in range(epochs):
            ...
            for batch_idx, batch in enumerate(progress_bar):
                try:
                    # Training step
                    loss = self.compute_aggressive_loss(batch)
            ...
            if (epoch + 1) % save_every == 0:
                checkpoint_path = output_dir / f"aggressive_flux_lora_epoch_{epoch+1}.pt"
        ...
        # Save final model
        final_path = output_dir / "aggressive_flux_lora_final.pt"
    ...
    def save_checkpoint(self, path, epoch, optimizer):
        ...
        try:
            ...
            for i, lora_layer in enumerate(self.lora_layers):
                ...
                # Log the magnitude encouragement
                b_magnitude = torch.mean(torch.abs(b_param))
                logger.info(f"Layer {i} - B magnitude: {b_magnitude:.6f}")
...
def main():
    parser = argparse.ArgumentParser(description="Aggressive Flux LoRA Training")
    parser.add_argument("--dataset_path", required=True, help="Path to dataset directory")
    parser.add_argument("--output_dir", default="./aggressive_flux_lora", help="Output directory")
    ...
    try:
        trainer = AggressiveFluxLoRATrainer(
