...
class FluxLoRATrainer:
    ...
    def setup_environment(self):
        ...
        if not self.sd_scripts_dir.exists():
            ...
            subprocess.run([
                "pip", "install", "-r", str(self.sd_scripts_dir / "requirements.txt")
            ], check=True)
        else:
...
"""
    ...
    def create_training_script(self, train_dir, model_name, output_name):
        ...
        t5_snapshots = list(t5_cache_base.glob("snapshots/*"))
        clip_snapshots = list(clip_cache_base.glob("snapshots/*"))
        ...
        # Use standalone CLIP and T5 models
        if clip_snapshots:
            clip_snapshot = clip_snapshots[0]
            clip_l_path = clip_snapshot  # Use directory, not specific file
        ...
        if t5_snapshots:
            ...
            t5xxl_path = t5_snapshot  # Use directory, not specific file
