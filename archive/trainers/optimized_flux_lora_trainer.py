...
class OptimizedLoRALayer(nn.Module):
    def __init__(self, original_layer, rank=4, alpha=16):
        ...
        logger.info(f"LoRA layer: {in_dim} -> {out_dim}, rank={rank}, alpha={alpha}, scaling={self.scaling}")
...
class OptimizedFluxLoRATrainer:
    def __init__(self, model_name="black-forest-labs/FLUX.1-schnell", device="cuda", torch_dtype=torch.float16):
        """
        Optimized Flux LoRA trainer with proper CPU offloading
        """
        self.device = device
        ...
        for pattern in target_patterns:
            for name, module in transformer.named_modules():
                if isinstance(module, nn.Linear) and pattern in name:
                    ...
                    # Create LoRA layer
                    lora_layer = OptimizedLoRALayer(module, rank=rank, alpha=alpha)
    ...
    def compute_lora_loss(self, batch):
        """Compute loss for LoRA training using text encoding"""
        
        caption = batch['caption'][0] if isinstance(batch['caption'], list) else batch['caption']
        
        # Use text encoder to get embeddings
        with torch.no_grad():
            # Get text embeddings
            ...
            # Move to appropriate device (CPU offload handles this)
            text_embeddings = self.pipe.text_encoder(text_inputs.input_ids.to(self.pipe.text_encoder.device))[0]
        
        # Simple reconstruction loss for LoRA parameters
        loss = torch.tensor(0.0, device=self.device, requires_grad=True)
        
        for lora_layer in self.lora_layers:
            # Encourage learning on target prompts
            if "anddrrew" in caption.lower():
                ...
                # Regularization
                loss = loss + torch.mean(lora_layer.lora_A ** 2) * 0.001
                loss = loss + torch.mean(lora_layer.lora_B ** 2) * 0.001
    ...
    def train(self, dataset_path, output_dir, epochs=50, batch_size=1, learning_rate=1e-4, 
        ...
        for epoch in range(epochs):
            ...
            if (epoch + 1) % save_every == 0:
                checkpoint_path = output_dir / f"optimized_flux_lora_epoch_{epoch+1}.pt"
                self.save_checkpoint(checkpoint_path, epoch, optimizer)
                logger.info(f"Saved checkpoint: {checkpoint_path}")
                
                # Test generation if validation prompt provided
        ...
        # Save final model
        final_path = output_dir / "optimized_flux_lora_final.pt"
        self.save_checkpoint(final_path, epochs-1, optimizer)
        logger.info(f"Training completed! Final model: {final_path}")
    
    def test_generation(self, prompt, output_path):
        """Test generation with current LoRA"""
        try:
...
def main():
    parser = argparse.ArgumentParser(description="Optimized Flux LoRA Training")
    parser.add_argument("--dataset_path", required=True, help="Path to dataset directory")
    parser.add_argument("--output_dir", default="./optimized_flux_lora", help="Output directory")
    ...
    parser.add_argument("--validation_prompt", default="anddrrew, portrait of a man", help="Validation prompt")
