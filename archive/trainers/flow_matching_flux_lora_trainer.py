...
class FlowMatchingFluxLoRATrainer:
    ...
    def __init__(self, args):
        ...
        logger.info("🚀 Initializing Flow Matching FLUX LoRA Trainer")
    ...
    def setup_lora(self):
        ...
        logger.info("🎯 Setting up LoRA for Flow Matching...")
    ...
    def train(self):
        ...
        logger.info("🎓 Starting Flow Matching FLUX LoRA training...")
        ...
        for epoch in range(self.args.num_epochs):
            ...
            if successful_batches > 0:
                ...
                logger.info(f"📊 Epoch {epoch + 1} complete - Average loss: {avg_loss:.6f} ({successful_batches}/{len(dataloader)} successful batches)")
            else:
                logger.warning(f"⚠️ Epoch {epoch + 1} - No successful batches!")
            
            # Save checkpoint
            if (epoch + 1) % 1 == 0:  # Save every epoch for now
                self.save_checkpoint(epoch + 1)
        
        logger.info("🎉 Flow Matching FLUX LoRA training complete!")
    ...
    def save_checkpoint(self, epoch):
        ...
        logger.info(f"💾 Saving Flow Matching LoRA checkpoint for epoch {epoch}...")
